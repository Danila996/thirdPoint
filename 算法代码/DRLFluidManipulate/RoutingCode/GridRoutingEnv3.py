...
class GridRoutingEnv(gym.Env, ABC):
    ...
    网格状态：
        cur_task + 0.1：当前任务的起点，例如第一个任务cur_task = 1，则网格上起点区域为1.1
        cur_task + 0.2：当前任务的起点，例如第一个任务cur_task = 1，则网格上起点区域为1.2
        2：任务所经过的路径网格
    """
    ...
    def __init__(self, base_grid, tasks, max_steps=500, reward_scale=1):
        ...
        for task in self.tasks:
            for pos in task["source_area"]:
                self.base_grid[pos[0], pos[1]] = cur_task * 10 + 1
            for pos in task["target_area"]:
                self.base_grid[pos[0], pos[1]] = cur_task * 10 + 2
            # seg0: 外部输入 -> 源区域
        ...
        self.obs = np.zeros(shape=(2, 10, 10), dtype=float)
        self.action_space = spaces.Discrete(self.num_tasks * 4)
        self.observation_space = spaces.Dict({
            "grid": spaces.Box(low=0, high=self.num_tasks * 10 + 2, shape=(self.rows, self.cols), dtype=np.int32),
            "agent_positions": spaces.Box(low=0, high=self.num_tasks * 10 + 2, shape=(self.rows, self.cols), dtype=np.int32)
        })
    ...
    def _get_observation(self):
        agent_pos = np.zeros((10, 10), dtype=np.int32)
        for task in self.tasks:
            seg = task["current_segment"]
            pos = task[f"seg{seg}"]["current_position"]
            # 如果未确定，则用 (-1, -1) 表示
            if pos is not None:
                agent_pos[pos[0], pos[1]] = 9
    ...
    def _in_allowed(self, task, pos, seg):
        ...
        if base_val in [cur_task * 10 + 1, cur_task * 10 + 2]:
        ...
        for task in self.tasks:
            ...
            if current_seg in [0, 1]:
                ...
                is_over = True if current_seg == 1 or (current_seg == 0 and
                                                       len(task[seg_key]["covered"]) < task["required_volume"]) else False
    ...
    def step(self, action):
        ...
        if seg in [0, 2] and seg_state["current_position"] is None:
            ...
            if pos is None:
                return self._get_observation(), self.routing_penalty * self.reward_scale, False, False, {"error": "未找到合适外部端"}
        ...
        if not self._in_allowed(task, new_pos, allowed_seg):
            return self._get_observation(), self.routing_penalty * self.reward_scale, False, False, {"error": "目标单元不允许或被占用"}
            ...
            if new_pos in task["source_area"]:
                ...
                if cur_pos in task["source_area"]:
                    if len(seg_state["covered"]) < task["required_volume"]:
                        ...
                        print(f"{seg:}mask_action invalid", seg_state["covered"], task["source_area"], task["required_volume"])
                        self.render()
        ...
        if self._check_deadlock():
            return self._get_observation(), -100 * self.reward_scale, True, False, {"deadlock": True}
        else:
    ...
    def render(self, mode="human"):
        ...
        for task in self.tasks:
            ...
            for pos in task["source_area"]:
                ...
                if self.base_grid[r, c] == cur_task * 10 + 1 and self.grid[r, c] == cur_task * 10 + 1:
                    grid_disp[r, c] = f"{cur_task}"
            for pos in task["target_area"]:
                r, c = pos
                if self.base_grid[r, c] == cur_task * 10 + 2 and self.grid[r, c] == cur_task * 10 + 2:
